...
When in headless mode, if the {\tt-r} or {\tt--run} options are not specified, only a {\tt complete\_model.jar} payload is created in a specified output directory and the model is not run. This is helpful for automated generation of payloads that are later run on remote resources (e.g., as seen in Sec. \ref{sec:hpc}, or when using workflow software such as the Swift/K or Swift/T parallel scripting languages\footnote{ http://swift-lang.org}).
