...
\end{center}
\end{figure}
The User Panel on the left reveals the graphical elements that we created (Fig.~\ref{fig:graphelem}). Pressing the \emph{Step} button at the top of the runtime (Fig.~\ref{fig:stepRuntime}) will advance the simulation schedule to time tick 0 and will, therefore, run the \emph{setup} method we scheduled with the \texttt{@Setup} annotation in the UserObserver (Listing~\ref{lst:observer2}). This should result in Figure~\ref{fig:setupzombies}.
