...
\item
Choose the UserObserver as the Agent Class and click on Next.
\item
Click Add to add a Simple Agent Mapping. Left click on \emph{current tick: double} in the source column in the second row and choose \emph{timestamp}() from the drop down list.
\item
Click Add again. The tick mapping should be added again. Choose \emph{getRelogoRun}\footnote{Groovy automatically generates ``getters" and ``setters" for properties you define. If the property is ``relogoRun" like in this example, the methods \emph{getRelogoRun} and \emph{setRelogoRun} are generated. This capitalization scheme, where the beginning of multiple contiguous words are capitalized, is a standard in many programming languages, including Groovy, and is referred to as ``camel-casing". } from the drop down list.
\item
Repeat Step 4, but choose \emph{remainingHumans} from the list.
\item
Repeat Step 4, but choose \emph{remainingZombies} from the drop down list.
\end{enumerate}
\begin{figure}
\begin{center}
\vspace{.2in}
...
Next we'll create a file outputter. To do this we carry out the following steps:

\begin{enumerate}
\item
Right click on Outputters label in the Scenario Tree (you may have to scroll down a bit).
\item
Click on \emph{timestamp} in File Columns and click the green right arrow.
\item
Repeat for the remaining items in the column.
\item
Click Next, then Finish (the default location for the file outputter is good for the purpose of this guide).
\end{enumerate}
...
\caption{The Spreadsheet wizard with the File Outputter selected.}
...
\begin{minipage}{\linewidth}
   ...
   \label{tab:graphical}
      \end{minipage}
\end{table}
Additionally, \emph{Observer}s are provided with a \emph{registerModelParameterListener(String,Closure)} method which allows for registering a closure to execute when a specified model parameter (such as those defined by the Global, Slider, Chooser, Switch and Input elements) are modified.
